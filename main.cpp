...
void display_3()
{
	...
	float zNear = camera_3->zNear;
	float zFar = camera_3->zFar;
	camera_3->projMatrix = camera_3->ortho(-camera_3->scale, camera_3->scale, -camera_3->scale, camera_3->scale, zNear, zFar);
...
void display_4()
{
	...
	float fov = camera_4->fov;
	float aspect = camera_4->aspect;
	float zNear = camera_4->zNear;
	float zFar = camera_4->zFar;
	camera_4->projMatrix = camera_4->perspective(fov, aspect, zNear, zFar);
	// 传递投影变换矩阵
	glUniformMatrix4fv(cube_object.modelLocation, 1, GL_FALSE, &modelMatrix[0][0]);
